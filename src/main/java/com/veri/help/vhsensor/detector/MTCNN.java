...
//    private static final float[] THRESHOLDS =  {0.2f, 0.4f, 0.6f};  //{0.6f, 0.7f, 0.7f}; //
//    private static final int MINSIZE = 50; // 35; //50; //20; //12;  //
//    private static final float[] THRESHOLDS = {0.6f, 0.7f, 0.8f};
...
//                faceBox.score = confidenceData[i];
    ...
    private float computeOverlap(FaceBox boxA, FaceBox boxB, char method){
        float iou;

        float xA = Math.max(boxA.xMin, boxB.xMin);
        float yA = Math.max(boxA.yMin, boxB.yMin);
        float xB = Math.min(boxA.xMax, boxB.xMax);
        float yB = Math.min(boxA.yMax, boxB.yMax);

        float interArea = Math.max(0, xB - xA + 1) * Math.max(0, yB - yA + 1);

        float boxAArea = (boxA.xMax - boxA.xMin + 1) * (boxA.yMax - boxA.yMin + 1);
        float boxBArea = (boxB.xMax - boxB.xMin + 1) * (boxB.yMax - boxB.yMin + 1);

        if (method == 'u'){
            iou = interArea / (float) (boxAArea + boxBArea - interArea);
        }
        else {
            iou = interArea / Math.min(boxAArea, boxBArea);
        }

        return iou;
    }

  private void getMaxScoreIndex(List<FaceInfo> bboxes, float threshold, int top_k,
        ...
        for (int i = 0; i < bboxes.size(); i++) {
            if (bboxes.get(i).bbox.score > threshold) {
                score_index_vec.add(new score_index_list(bboxes.get(i).bbox.score, i));
            }
        }
    ...
    private List<Integer> NMSFaster_new(List<FaceInfo> bboxes, float score_threshold,
                                     float nms_threshold, float eta, int top_k, char method) {
        ...
        getMaxScoreIndex(bboxes, score_threshold, top_k, score_index_vec);
        ...
        for (int i = 0; i < score_index_vec.size(); i++)
        {
            ...
            for (int k = 0; k < indices.size() && keep; ++k)
            {
                ...
                float overlap = computeOverlap(bboxes.get(idx).bbox, bboxes.get(kept_idx).bbox, method);
...
//            int lastIndex = indices.size() - 1;
    ...
    private List<FaceInfo> NMS_NEW(List<FaceInfo> bboxes, float threshold, char method) {
        long detectStart = System.currentTimeMillis();

        List<Integer> indices = NMSFaster_new(bboxes,0.01f, threshold, 1.0f, 0, method);
        ...
        long detectEnd = System.currentTimeMillis();
        Log.d(TAG, "Time Taken for new faster nms - " + (detectEnd - detectStart) + "ms");
        return bboxesNms;
    }
...
//            List<FaceInfo> bboxesNMS = NMSFaster(candidateBoxes, prob, 0.5f, nms_threshold,  'u');
        ...
        if (numBox != 0) {
            ...
            resBoxes = NMS(totalBoxes, 0.7f, 'u');  //modified from 0.7f
            nmsLatency += (System.currentTimeMillis() - nmsStart);
...
//            RNetResult = NMS(RNetResult, 0.4f, 'm');
            RNetResult = NMS(RNetResult, 0.7f, 'u');  //modified by saras
...
//            ONetResult = NMS(ONetResult, 0.4f, 'm');
            ONetResult = NMS(ONetResult, 0.7f, 'm');  //modified by saras
